...
/*
 ...
 Uncomment when credentials expiration environment overrides is implemented
...
static int s_profile_file_credentials_provider_get_credentials_async(
    ...
    struct aws_credentials_provider_profile_file_impl *impl =
        (struct aws_credentials_provider_profile_file_impl *)provider->impl;
...
static void s_profile_file_credentials_provider_clean_up(struct aws_credentials_provider *provider) {
    ...
    if (impl->config_file_path) {
        aws_string_destroy(impl->config_file_path);
    }

    if (impl->credentials_file_path) {
...
struct aws_credentials_provider *aws_credentials_provider_new_profile(
    ...
    struct aws_credentials_provider_profile_file_impl *impl =
        (struct aws_credentials_provider_profile_file_impl *)aws_mem_acquire(
            allocator, sizeof(struct aws_credentials_provider_profile_file_impl));
    if (impl == NULL) {
        return NULL;
    }
    AWS_ZERO_STRUCT(*impl);
    ...
    struct aws_credentials_provider *provider =
        (struct aws_credentials_provider *)aws_mem_acquire(allocator, sizeof(struct aws_credentials_provider));
    if (provider == NULL) {
        goto on_provider_allocate_failure;
    }

    struct aws_string *credentials_file_path =
        aws_get_credentials_file_path(allocator, options->credentials_file_name_override);
    if (credentials_file_path == NULL) {
        AWS_LOGF_ERROR(
            AWS_LS_AUTH_CREDENTIALS_PROVIDER,
            "Profile credentials provider (%p) failed resolve credentials file path",
            (void *)provider);
        goto on_credentials_path_resolution_failure;
    }

    struct aws_string *config_file_path = aws_get_config_file_path(allocator, options->config_file_name_override);
    if (config_file_path == NULL) {
        AWS_LOGF_ERROR(
            AWS_LS_AUTH_CREDENTIALS_PROVIDER,
            "Profile credentials provider (%p) failed resolve config file path",
            (void *)provider);
        goto on_config_path_resolution_failure;
    }

    struct aws_string *profile_name = aws_get_profile_name(allocator, options->profile_name_override);
    if (profile_name == NULL) {
        AWS_LOGF_ERROR(
            AWS_LS_AUTH_CREDENTIALS_PROVIDER,
            "Profile credentials provider (%p) failed to resolve profile name",
            (void *)provider);
        goto on_profile_name_resolution_failure;
    }

    impl->credentials_file_path = credentials_file_path;
    impl->config_file_path = config_file_path;
    impl->profile_name = profile_name;
...
on_profile_name_resolution_failure:
    aws_string_destroy(config_file_path);

on_config_path_resolution_failure:
    aws_string_destroy(credentials_file_path);

on_credentials_path_resolution_failure:
    aws_mem_release(allocator, provider);

on_provider_allocate_failure:
    aws_mem_release(allocator, impl);
...
on_provider_list_copy_failure:
    aws_array_list_clean_up(&impl->providers);

on_provider_list_init_failure:
    aws_mem_release(allocator, provider);
 ...
 *
 * We don't currently have implementations of (3) and (4)
 *
 */
struct aws_credentials_provider *aws_credentials_provider_new_chain_default(struct aws_allocator *allocator) {
     ...
     * environment
     */
    struct aws_credentials_provider *environment_provider = aws_credentials_provider_new_environment(allocator);
    ...
    AWS_ZERO_STRUCT(profile_provider_options);

    struct aws_credentials_provider *profile_provider =
        aws_credentials_provider_new_profile(allocator, &profile_provider_options);
    if (profile_provider == NULL) {
        goto on_new_profile_provider_failure;
    }

    /*
     * Build a chain from the individual pieces
     */
    struct aws_credentials_provider *providers[2] = {environment_provider, profile_provider};

    struct aws_credentials_provider_chain_options chain_options = {.providers = providers, .provider_count = 2};

    struct aws_credentials_provider *chain_provider = aws_credentials_provider_new_chain(allocator, &chain_options);
    if (chain_provider == NULL) {
        goto on_new_chain_provider_failure;
    }

    /*
     * Decorate with a cached provider
     */
    struct aws_credentials_provider_cached_options cache_options = {.source = chain_provider,
                                                                    .refresh_time_in_milliseconds =
                                                                        DEFAULT_CREDENTIALS_CACHE_REFRESH_TIME_MS,
                                                                    .clock_fn = NULL};

    struct aws_credentials_provider *cached = aws_credentials_provider_new_cached(allocator, &cache_options);
    if (cached == NULL) {
        goto on_new_cached_provider_failure;
    }

    return cached;

on_new_cached_provider_failure:
    aws_credentials_provider_destroy(chain_provider);

on_new_chain_provider_failure:
    aws_credentials_provider_destroy(profile_provider);

on_new_profile_provider_failure:
